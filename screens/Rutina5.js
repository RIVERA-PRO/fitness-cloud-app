...
export default function Rutina5() {
    ...
    useEffect(() => {
        const getFavorites = async () => {
            try {
                const favoritesData = await AsyncStorage.getItem('Dia5');
                if (favoritesData) {
                    setFavorites(JSON.parse(favoritesData));
                }
            } catch (error) {
                console.log('Error getting Dia5:', error);
            }
        };
    ...
    const removeExercise = async (exerciseId) => {
        try {
            ...
            await AsyncStorage.setItem('Dia5', JSON.stringify(updatedFavorites));
            setFavorites(updatedFavorites);
        } catch (error) {
            console.log('Error removing Dia5:', error);
        }
    };

    const removeAllExercises = async () => {
        try {
            await AsyncStorage.removeItem('Dia5');
            setFavorites([]);
        } catch (error) {
            console.log('Error removing all Dia5:', error);
        }
    };
            ...
            {favorites.length > 0 ? (
                ...
                <View>
                    ...
                    <LinearGradient colors={['#AC1929', '#D71920',]} style={styles.containerInput}>
                        ...
                        <View style={styles.titleCantidad}>
                            <Text style={styles.textoTitle}> Ejercicios del Dia 5</Text>
                ...
                <>
                    ...
                    <ImageBackground source={imagebg} style={styles.imagebg} resizeMode="cover" >
                        <View style={styles.noHayFavoritos}>
                            <Text style={styles.noHayFavoritos}>No hay Ejercicios del Día 5</Text>
                        </View>
