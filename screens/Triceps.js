...
import tricepsImg from '../assets/triceps3.jpeg'
import Icon from 'react-native-vector-icons/MaterialIcons';
import { LinearGradient } from 'expo-linear-gradient';
export default function Triceps({ navigation }) {
    const [searchText, setSearchText] = useState('');
    const tricepsExercises = ejerciciosData.filter(exercise => exercise.categoria === 'Triceps' && exercise.title.toLowerCase().includes(searchText.toLowerCase()));

    const goToDetail = (exerciseId) => {
        navigation.navigate('Detail', { exerciseId });
    };
    const showNoResults = tricepsExercises.length === 0;
    return (
        ...
        <ScrollView contentContainerStyle={styles.scrollContainer}>
            ...
            <View style={styles.imgBg}>
                <Image source={tricepsImg} style={styles.img} />
            </View>
            ...
            <View style={styles.seccion}>
                ...
                <View style={styles.titleCantidad}>
                    <Text style={styles.textoTitle}>Ejercicios de Triceps</Text>
                    <Text style={styles.textoCantidad}>{tricepsExercises.length}</Text>
                </View>
