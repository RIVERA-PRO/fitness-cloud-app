...
export default function Rutina7() {
    ...
    useEffect(() => {
        const getFavorites = async () => {
            try {
                const favoritesData = await AsyncStorage.getItem('Dia7');
                if (favoritesData) {
                    setFavorites(JSON.parse(favoritesData));
                }
            } catch (error) {
                console.log('Error getting Dia7:', error);
            }
        };
    ...
    const removeExercise = async (exerciseId) => {
        try {
            ...
            await AsyncStorage.setItem('Dia7', JSON.stringify(updatedFavorites));
            setFavorites(updatedFavorites);
        } catch (error) {
            console.log('Error removing Dia7:', error);
        }
    };

    const removeAllExercises = async () => {
        try {
            await AsyncStorage.removeItem('Dia7');
            setFavorites([]);
        } catch (error) {
            console.log('Error removing all Dia7:', error);
        }
    };
            ...
            {favorites.length > 0 ? (
                ...
                <View>
                    ...
                    <LinearGradient colors={['#AC1929', '#D71920',]} style={styles.containerInput}>
                        ...
                        <View style={styles.titleCantidad}>
                            <Text style={styles.textoTitle}> Ejercicios del Dia 7</Text>
                ...
                <>
                    ...
                    <ImageBackground source={imagebg} style={styles.imagebg} resizeMode="cover" >
                        <View style={styles.noHayFavoritos}>
                            <Text style={styles.noHayFavoritos}>No hay Ejercicios del Día 7</Text>
                        </View>
