...
export default function Rutina() {
    ...
    useEffect(() => {
        const getFavorites = async () => {
            try {
                const favoritesData = await AsyncStorage.getItem('Dia1');
                if (favoritesData) {
    ...
    const removeExercise = async (exerciseId) => {
        try {
            ...
            await AsyncStorage.setItem('Dia1', JSON.stringify(updatedFavorites));
            setFavorites(updatedFavorites);
        } catch (error) {
            console.log('Error removing Dia1:', error);
        }
    };

    const removeAllExercises = async () => {
        try {
            await AsyncStorage.removeItem('Dia1');
            setFavorites([]);
        } catch (error) {
            ...
            {favorites.length > 0 ? (
                ...
                <View>
                    ...
                    <LinearGradient colors={['#AC1929', '#D71920',]} style={styles.containerInput}>
                        ...
                        <View style={styles.titleCantidad}>
                            <Text style={styles.textoTitle}> Ejercicios del Dia 1</Text>
                ...
                <>
                    ...
                    <ImageBackground source={imagebg} style={styles.imagebg} resizeMode="cover" >
                        <View style={styles.noHayFavoritos}>
                            <Text style={styles.noHayFavoritos}>No hay Ejercicios del Día 1</Text>
                        </View>
